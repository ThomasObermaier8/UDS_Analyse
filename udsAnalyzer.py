#!/usr/bin/env python3
"""UDS Analyzer GUI with embedded protocol database and DB viewer."""
...
UDS_DATABASE = {
    "services": {
        ...
        0x85: {"name": "ControlDTCSetting", "subfunctions": {0x01: "on", 0x02: "off"}},
        0x86: {"name": "ResponseOnEvent"},
        0x87: {"name": "LinkControl"},
    },
    "negative_response_codes": {
        0x10: "generalReject", 0x11: "serviceNotSupported", 0x12: "subFunctionNotSupported", 0x13: "incorrectMessageLengthOrInvalidFormat", 0x14: "responseTooLong", 0x21: "busyRepeatRequest", 0x22: "conditionsNotCorrect", 0x24: "requestSequenceError", 0x25: "noResponseFromSubnetComponent", 0x26: "failurePreventsExecutionOfRequestedAction", 0x31: "requestOutOfRange", 0x33: "securityAccessDenied", 0x35: "invalidKey", 0x36: "exceedNumberOfAttempts", 0x37: "requiredTimeDelayNotExpired", 0x38: "secureDataTransmissionRequired", 0x39: "secureDataTransmissionNotAllowed", 0x3A: "secureDataVerificationFailed", 0x70: "uploadDownloadNotAccepted", 0x71: "transferDataSuspended", 0x72: "generalProgrammingFailure", 0x73: "wrongBlockSequenceCounter", 0x78: "requestCorrectlyReceivedResponsePending", 0x7E: "subFunctionNotSupportedInActiveSession", 0x7F: "serviceNotSupportedInActiveSession", 0x81: "rpmTooHigh", 0x82: "rpmTooLow", 0x83: "engineIsRunning", 0x84: "engineIsNotRunning", 0x85: "engineRunTimeTooLow", 0x86: "temperatureTooHigh", 0x87: "temperatureTooLow", 0x88: "vehicleSpeedTooHigh", 0x89: "vehicleSpeedTooLow", 0x8A: "throttlePedalTooHigh", 0x8B: "throttlePedalTooLow", 0x8C: "transmissionRangeNotInNeutral", 0x8D: "transmissionRangeNotInGear", 0x8F: "brakeSwitchNotClosed", 0x90: "shifterLeverNotInPark", 0x91: "torqueConverterClutchLocked", 0x92: "voltageTooHigh", 0x93: "voltageTooLow",
    },
    "did_notes": {
...
def get_subfunction_name(sid: int, sub: int) -> str:
    service = UDS_DATABASE["services"].get(sid, {})
    return service.get("subfunctions", {}).get(sub, "unknownSubFunction")
...
def bytes_to_ascii(byte_values: list[int]) -> str:
    ...
    for value in byte_values:
        if 32 <= value <= 126:
            ...
            chars.append(".")
    if printable_count / len(byte_values) < 0.6:
        return ""
    return "".join(chars)
...
def build_database_entries() -> list[str]:
    ...
    for sid, service in sorted(UDS_DATABASE["services"].items()):
        entries.append(f"SERVICE 0x{sid:02X} {service['name']}")
...
def filter_database_entries(entries: list[str], query: str) -> list[str]:
    ...
    if not q:
        return entries
    return [entry for entry in entries if q in entry.lower()]
...
def decode_payload(payload: list[int]) -> str:
    ...
    if sid == 0x7F:
        ...
        parts.append(f"NRC: {UDS_DATABASE['negative_response_codes'].get(nrc, 'unknownNRC')} (0x{nrc:02X})")
        if len(payload) > 3:
            extra = payload[3:]
    ...
    if len(payload) >= 2 and UDS_DATABASE["services"].get(base_sid, {}).get("subfunctions"):
    ...
    if base_sid in (0x22, 0x2E) and len(payload) >= 3:
        ...
        if len(payload) > 3:
            did_ascii = bytes_to_ascii(payload[3:])
            if did_ascii:
...
class UDSAnalyzerApp(tk.Tk):
    def __init__(self) -> None:
        ...
        ttk.Label(self, text="UDS Kommunikation Decoder (mit eingebetteter Datenbank)", font=("Arial", 14, "bold")).grid(row=0, column=0, padx=10, pady=(10, 6), sticky="w")
    ...
    def open_database_viewer(self) -> None:
        ...
        self.db_window.title("UDS Datenbank – Suche")
        self.db_window.geometry("900x600")
        self.db_window.columnconfigure(0, weight=1)
        self.db_window.rowconfigure(2, weight=1)

        ttk.Label(self.db_window, text="Suche (Hex, Name, Kategorie):").grid(row=0, column=0, padx=10, pady=(10, 4), sticky="w")
        search_entry = ttk.Entry(self.db_window, textvariable=self.db_search_var)
        search_entry.grid(row=1, column=0, padx=10, pady=(0, 6), sticky="ew")
        search_entry.bind("<KeyRelease>", lambda _event: self.refresh_database_view())
    ...
    def refresh_database_view(self) -> None:
        ...
        if filtered:
            ...
            self.db_text.insert("1.0", "Keine Einträge für die aktuelle Suche gefunden.")
